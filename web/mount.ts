...
function organizationView(
  ...
  assignmentOptions: Awaited<ReturnType<CompanyOSApplicationClient["assignmentOptions"]>>,
): string {
  ...
  const agentDetails = organization.agents.map((agent) => {
    ...
    const runtime = administration?.runtimeConnectors.find(({ connectorId }) => connectorId === selectedConnectorId);
    const contract = assignmentOptions.responsibilities.contracts.find(({ agentId }) => agentId === agent.id);
    const lifecycle = assignmentOptions.lifecycle.agents.find(({ agentId }) => agentId === agent.id);
    const toolBindings = administration?.toolAccess.bindings.filter(({ targetType, targetId }) =>
      targetType === "company" && targetId === organization.company.id || targetType === "agent" && targetId === agent.id) ?? [];
    const dataContracts = administration?.governance.dataAuthorizationContracts.filter(({ status, authorizedAgentIds }) =>
      status === "ACTIVE" && authorizedAgentIds.includes(agent.id)) ?? [];
    const readinessItems = [{ ready: organization.humans.some(({ id }) => id === agent.accountableHumanId),
      label: copy("Accountable human", "真人负责人"), next: "organization",
      detail: copy("Assign a verified human owner.", "分配已验证的真人负责人。") },
    { ready: contract?.status === "ACTIVE", label: copy("Responsibility contract", "责任合同"), next: "agents",
      detail: copy("Review action policy and activate responsibility.", "复核动作策略并启用责任合同。") },
    { ready: bindingStatus === "VERIFIED" && runtime?.health === "HEALTHY", label: copy("Runtime binding", "运行环境绑定"), next: "connectors",
      detail: copy("Bind a healthy discovered Runtime and verify its capabilities.", "绑定健康的已发现 Runtime，并校验其能力。") },
    { ready: lifecycle?.eligibility.invokable === true, label: copy("Lifecycle & organization chain", "生命周期与组织链"), next: "agents",
      detail: copy("Approve the Agent and resolve every organization-chain blocker.", "批准 Agent，并处理所有组织链阻塞项。") }];
    const executable = readinessItems.every(({ ready }) => ready);
    const readiness = `<section class="agent-readiness" aria-labelledby="readiness-${escapeHtml(agent.id)}"><header><div><p class="family-kicker">${copy("EXECUTION READINESS", "执行就绪度")}</p><h3 id="readiness-${escapeHtml(agent.id)}">${executable ? copy("Ready for governed work", "可执行受治理任务") : copy("Complete these prerequisites", "请完成以下前置条件")}</h3></div><span class="status-pill ${executable ? "status-pill--demo" : "status-pill--unbound"}">${readinessItems.filter(({ ready }) => ready).length}/${readinessItems.length}</span></header><ol>${readinessItems.map(({ ready, label, next, detail }) => `<li data-readiness-state="${ready ? "ready" : "blocked"}"><span aria-hidden="true">${ready ? "✓" : "!"}</span><div><strong>${label}</strong><small>${ready ? copy("Verified", "已验证") : detail}</small></div>${ready ? "" : `<button type="button" data-section-target="${next}">${copy("Fix now", "立即处理")}</button>`}</li>`).join("")}</ol><dl><div><dt>${copy("Tool authority", "工具权限")}</dt><dd>${toolBindings.length ? `${toolBindings.length} ${copy("effective profile bindings", "项生效配置绑定")}` : copy("No tool grant requested; default deny", "未申请工具授权；默认拒绝")}</dd></div><div><dt>${copy("Data authority", "数据权限")}</dt><dd>${dataContracts.length ? `${dataContracts.length} ${copy("active contracts", "项生效合同")}` : copy("No enterprise data contract", "无企业数据授权合同")}</dd></div></dl></section>`;
    const bindingForm = isFormal ? `${readiness}<form class="colleague-profile-form agent-runtime-binding-form" data-agent-runtime-binding-form data-agent-id="${escapeHtml(agent.id)}"><h3>${copy("Runtime binding", "运行环境绑定")}</h3><p>${copy("Create the Agent first, then attach a discovered runtime. Every bind, rebind, and unbind is authorized and revision checked.", "先创建 Agent，再绑定已发现的运行环境。绑定、换绑和解绑都会经过授权与版本校验。")}</p><label class="company-field">${copy("Available runtime", "可用运行环境")}<select class="company-form-control" name="connectorId"${bindableConnectors.length ? "" : " disabled"}>${runtimeOptions}</select></label><label class="company-field">${copy("Reason", "变更原因")}<input class="company-form-control" name="reason" required maxlength="1000" placeholder="${copy("Why this runtime is appropriate", "说明为何选择此运行环境")}"></label><footer><button class="family-button family-button--primary" type="submit" name="operation" value="BIND"${bindableConnectors.length ? "" : " disabled"}>${selectedConnectorId ? copy("Review and rebind", "审核并换绑") : copy("Review and bind", "审核并绑定")}</button>${selectedConnectorId ? `<button class="family-button family-button--danger" type="submit" name="operation" value="UNBIND">${copy("Review and unbind", "审核并解绑")}</button>` : ""}</footer><input type="hidden" name="expectedRevision" value="${bindingRevision}"></form>` : "";
    const editDepartmentOptions = organization.departments.map((candidate) =>
  ...
  async function render(): Promise<void> {
    ...
    if (isDemo && publicDemoSnapshot && portfolioSection) {
      ...
      case "organization": main = organizationView(
        ...
        administration, assignmentOptions,
      ); break;
