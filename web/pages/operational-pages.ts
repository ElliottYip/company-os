...
export function evidencePage(
  ...
  const rows = entries.map((entry, index) => `<button type="button" class="product-record-row evidence-record-row" data-evidence-detail="${escapeHtml(entry.id)}"><span class="record-monogram">${"kind" in entry && entry.kind === "RESULT" ? "R" : `E${index + 1}`}</span><span><small>${"kind" in entry ? escapeHtml(entry.kind) : c(locale, "ADMITTED EVIDENCE", "有效证据")}</small><strong>${escapeHtml(entry.id)}</strong>${"contentDigest" in entry ? `<span class="evidence-record-meta"><code>${escapeHtml(entry.contentDigest)}</code><span>${escapeHtml(entry.source)}</span><time>${escapeHtml(entry.recordedAt)}</time></span>` : `<em>${c(locale, "Bound to work, Agent, provenance and responsibility record", "已关联任务、Agent、来源和责任记录")}</em>`}</span><span class="status-pill ${"kind" in entry && entry.kind === "RESULT" ? "status-pill--human" : "status-pill--demo"}">${state.mode === "DEMO_FIXTURE" ? c(locale, "VIEW FIXTURE", "查看演示证据") : c(locale, "VIEW", "查看")}</span></button>`).join("");
  const dialogs = entries.map((entry) => `<dialog class="evidence-detail-dialog family-overlay family-drawer" data-evidence-detail-dialog="${escapeHtml(entry.id)}" aria-labelledby="evidence-detail-${escapeHtml(entry.id)}"><header><div><p class="family-kicker">${"kind" in entry ? escapeHtml(entry.kind) : c(locale, "ADMITTED EVIDENCE", "有效证据")}</p><h2 id="evidence-detail-${escapeHtml(entry.id)}">${escapeHtml(entry.id)}</h2></div><button class="family-overlay-close" type="button" data-detail-close aria-label="${c(locale, "Close evidence", "关闭证据")}">×</button></header><div class="evidence-detail-body"><p>${"summary" in entry ? escapeHtml(entry.summary) : c(locale, "This deterministic fixture is bound to the current work and responsibility record.", "该确定性演示证据已绑定当前任务与责任记录。")}</p><dl><div><dt>${c(locale, "Work", "任务")}</dt><dd>${"workId" in entry ? escapeHtml(entry.workId) : escapeHtml(state.responsibility.workId)}</dd></div><div><dt>${c(locale, "Digest", "摘要")}</dt><dd>${"contentDigest" in entry ? `<code>${escapeHtml(entry.contentDigest)}</code>` : "—"}</dd></div><div><dt>${c(locale, "Source", "来源")}</dt><dd>${"source" in entry ? escapeHtml(entry.source) : c(locale, "Deterministic fixture", "确定性演示数据")}</dd></div><div><dt>${c(locale, "Recorded", "记录时间")}</dt><dd>${"recordedAt" in entry ? escapeHtml(entry.recordedAt) : "—"}</dd></div></dl><p class="profile-boundary-note">${c(locale, "Read-only evidence view. Admission, provenance and responsibility checks remain authoritative.", "只读证据视图；接纳、来源与责任校验仍以权威记录为准。")}</p></div></dialog>`).join("");
  return `<section class="page-stage product-list-page" data-section="evidence" aria-labelledby="evidence-title">
    ...
    <section class="admin-surface product-list-surface">${entries.length ? rows : emptyState(c(locale, "No admitted evidence yet", "暂无有效证据"), c(locale, "Ordinary attachments do not become evidence until provenance, digest, and responsibility checks pass.", "附件通过来源、摘要和责任校验后，才会进入证据记录。"))}${!ledger && state.responsibility.resultId ? `<article class="product-record-row product-record-row--static"><span class="record-monogram">R</span><span><small>${c(locale, "VERIFIED RESULT", "已核验结果")}</small><strong>${escapeHtml(state.responsibility.resultId)}</strong><em>${c(locale, "Traceable to goal, Agent, human decision and evidence", "可追溯到目标、Agent、审批人和证据")}</em></span><span class="status-pill status-pill--human">${c(locale, "RESULT", "结果")}</span></article>` : ""}</section>${dialogs}
  </section>`;
}
