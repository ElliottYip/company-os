...
test("staging is site-rendered, isolated, dependency-bound, resource bounded, and file-secret based", async () => {
  ...
  assert.match(example, /^COMPANY_OS_OIDC_REDIRECT_URI=https:\/\/CHANGE_ME_SITE_API_ORIGIN\/api\/auth\/oauth2\/callback\/enterprise-oidc$/m);
  const dependencyManifest = JSON.parse(dependencies);
  assert.equal(dependencyManifest.ingress.webOrigin, "https://company-os.raft.xin");
  assert.equal(dependencyManifest.ingress.apiOrigin, "https://company-os-api.raft.xin");
  assert.match(runbook, /Public Demo: `https:\/\/anc\.raft\.xin`/);
  assert.match(runbook, /Private Alpha Web: `https:\/\/company-os\.raft\.xin` -> `127\.0\.0\.1:4600`/);
  assert.match(runbook, /Private Alpha API: `https:\/\/company-os-api\.raft\.xin` -> `127\.0\.0\.1:4601`/);
  ...
  assert.equal(dependencyManifest.schemaVersion, 1);
...
test("staging first install has read-only diagnostics, exact handoff, prepare-only store, and authorized start", async () => {
  ...
  assert.match(installer, /SITE_CONTRACT_ADOPTION_PLANNED_NOT_APPLIED/);
  assert.match(runbook, /--adopt-site-contract/);
  assert.match(runbook, /does not create Secret values, initialize dependencies, migrate,/);
