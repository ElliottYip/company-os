...
test("portable staging archive contains only the verified handoff allowlist", async (context) => {
  ...
  assert.deepEqual(await listPortableStagingArchive(archive), [
    ...
    "compose.private-alpha-paperclip.yml", "compose.public-demo.yml", "compose.staging-dependencies.yml", "compose.staging-upgrade-candidate.yml",
    "compose.staging.yml", "customer-boundary-acceptance.md",
    "formal-identity-runbook.md", "paperclip-federated-alpha-runbook.md", "release-manifest.json", "staging-dependencies.Caddyfile",
