...
test("instance admin freezes and reopens dispatch through revisioned durable state", async () => {
  ...
  const open = await manager.execute({ mode: "OPEN", expectedRevision: 1,
    operationId: "upgrade-staging-01", authorizationReference: "change:approved-upgrade-01" });
  assert.equal(open.mode, "OPEN");
  assert.equal(open.revision, 2);
  assert.deepEqual(store.history.map(({ mode }) => mode), ["DISPATCH_FROZEN", "OPEN"]);
});
