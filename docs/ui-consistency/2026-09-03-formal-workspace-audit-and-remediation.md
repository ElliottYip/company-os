...
| A-009 | Whole Web | Token documentation existed, but a later page could still introduce raw font metrics or a one-off breakpoint without failing CI. | A repository-wide visual-contract gate now rejects raw text size/weight/leading, undocumented breakpoints, page-level negative margins and loss of the shared centered lane; E2E checks computed typography in both locales. |
| A-010 | 1440px formal workspace / 1024px Settings | The first RC33 live sweep found three legacy root selectors overriding the centered lane at wide desktop, plus General and Identity rows extending past the visible Settings content column at tablet width. | Organization, Accountability and Administration preserve the shared centered lane; the 1080px Settings contract reflows summary and member rows before the rail-constrained content width can clip them. The visual-contract gate rejects future page-root margin overrides and browser E2E visits every primary page at 1440px plus every Settings tab at 1024px. |

Before evidence is stored in `output/ui-audit-2026-09-03-alignment-pass2-before/`, `output/ui-audit-2026-09-03-rc31-alignment/`, and `output/audit-2026-09-03-company-secondary-rc31/`. The complete English/Chinese after matrix is stored in `output/playwright/ui-consistency-refinement-after/` (120 screenshots). Follow-up browser tests assert title/surface edge equality, symmetric gutters, compact-form containment, short-window focused-control visibility, modal centering, drawer pinning, typography inheritance and horizontal-overflow absence. `npm run verify` passes with 763 unit/static tests admitted (4 skipped by contract) and 31 browser E2E scenarios passed (5 environment-specific scenarios skipped).
