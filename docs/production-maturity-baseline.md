...
`npm test` passed on the exact qualified Draft PR #3 head with 661 cases (657
passed and four explicit environment-gated live cases skipped in the
credential-free run). The live
...
The candidate release manifest now binds the exact public source repository, SemVer tag,
release workflow and GitHub Actions qualification run/attempt URI alongside the
six image digests. The manifest therefore points to the run that produced its
...
authorization. Independent acceptance confirmation does not mutate dispatch;
a separate authorization and audit record either reopens normal dispatch after
acceptance or returns a rejected operation to `DISPATCH_FROZEN`. The same human
may hold both roles, but cannot collapse the commands or reuse their authority.
Existing admitted work continues to drain, so the control is not misrepresented
as a Connector kill switch.
...
|---|---|---|---|
| Independent runtime | Boundary and independence guards; no Paperclip package or schema dependency; immutable six-image `v0.1.0-rc.4` is published and independently attested; exact post-RC4 Draft PR #3 head `93ea16d57b14978c8c4efc6afaecd64a64c40606` passed the full GitHub Actions qualification | Merge the qualified source, publish a later immutable six-image RC, and repeat target validation without rewriting RC4 evidence | RC4 released; post-RC4 source qualified; later release pending |
| PostgreSQL storage | Seven ordered, non-destructive migrations with an advisory lock; durable event/outbox/checkpoint/backup adapters; current-run PostgreSQL 16 concurrent-migration/restart/tenant/backup admission passed locally with no skips and in the exact source-candidate CI; a second live admission provisions a distinct runtime role, proves required Company OS DML, and proves PostgreSQL denies schema creation/alteration, role creation, truncate and temporary-table authority; the support policy now admits current-minor PostgreSQL 16.15 and 17.11 only, pins both official image manifests, and a real 16→17 logical dump/restore preserved the migration journal and Company OS marker while leaving the 16 source unchanged as a parallel rollback boundary | Repeat the support admission whenever an upstream minor changes and execute it in the released target environment | Dual-major repository and CI compatibility complete; deployment evidence pending |
